...
func main() {
	...
	fmt.Println("Verifying user_preferences table...")

	// 1. Check if user_preferences table exists
	fmt.Println("[1] Checking user_preferences table structure...")
	tableCheckQuery := `
	...
	if err != nil {
		log.Printf("WARN: failed to check constraints: %v", err)
	} else {
	...
	if err != nil {
		log.Printf("WARN: failed to check triggers: %v", err)
	} else {
	...
	if count > 0 {
		...
		if err != nil {
			log.Printf("WARN: failed to get samples: %v", err)
		} else {
	...
	if err != nil {
		fmt.Printf("   ERROR: GetUserPreferences failed: %v\n", err)
	} else {
		fmt.Printf("   OK: GetUserPreferences | User ID: %d | Accent: %s | Theme: %s\n", prefs.UserID, prefs.AccentColor, prefs.Theme)
	}

	// Test UpdateUserPreferences
	err = database.UpdateUserPreferences(ctx, testUserID, "purple", "dark", true, true, true)
	if err != nil {
		fmt.Printf("   ERROR: UpdateUserPreferences failed: %v\n", err)
	} else {
		fmt.Println("   OK: UpdateUserPreferences")

		// Verify update
		updatedPrefs, err := database.GetUserPreferences(ctx, testUserID)
		if err != nil {
			fmt.Printf("   WARN: failed to verify update: %v\n", err)
		} else {
			if updatedPrefs.AccentColor == "purple" && updatedPrefs.Theme == "dark" {
				fmt.Println("   OK: update verified")
			} else {
				fmt.Printf("   WARN: update verification failed: accent=%s theme=%s\n", updatedPrefs.AccentColor, updatedPrefs.Theme)
			}
		}
	}

	// Reset to default for testing
	_ = database.UpdateUserPreferences(ctx, testUserID, "blue", "system", true, true, true)
