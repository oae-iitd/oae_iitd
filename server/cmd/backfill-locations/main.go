...
func main() {
	...
	if len(sessions) == 0 {
		fmt.Println("No sessions need location backfill")
		return
	}
	...
	for i, session := range sessions {
		...
		if err != nil {
			log.Printf("ERROR: failed to update session %d: %v", session.ID, err)
			failedCount++
			continue
		}
	...
	fmt.Printf("\nSummary:\n")
	fmt.Printf("   OK: updated %d sessions\n", updatedCount)
	if failedCount > 0 {
		fmt.Printf("   ERROR: failed %d sessions\n", failedCount)
	}
	fmt.Println("\nBackfill completed.")
}
