...
type config struct {
	...
	smtpFromName      string
	smsOtpTemplate    string // OTP message template (one %s for OTP)
	twilioAccountSid  string
	twilioAuthToken   string // primary: use for Account SID + Auth Token auth
	twilioAPIKey      string
	twilioAPISecret   string
	twilioPhoneNumber string
	appScheme         string // e.g. "mobile" for mobile:// deep links
	playStoreURL      string
	appStoreURL       string
}
...
func Init() {
	...
	if smtpFromName == "" {
		smtpFromName = "OAE IITD" // Default from name
	}

	smsOtpTemplate := strings.TrimSpace(os.Getenv("SMS_OTP_TEMPLATE"))
	if smsOtpTemplate == "" {
		smsOtpTemplate = "Your OTP for driver login is %s. Valid for 5 minutes."
	}
	...
	twilioPhoneNumber := strings.TrimSpace(os.Getenv("TWILIO_PHONE_NUMBER"))

	appScheme := strings.TrimSpace(os.Getenv("APP_SCHEME"))
	if appScheme == "" {
		appScheme = "mobile"
	}
	playStoreURL := strings.TrimSpace(os.Getenv("PLAY_STORE_URL"))
	if playStoreURL == "" {
		playStoreURL = "https://play.google.com/store/apps/details?id=com.anonymous.mobile"
	}
	appStoreURL := strings.TrimSpace(os.Getenv("APP_STORE_URL"))
	if appStoreURL == "" {
...
// AppScheme returns the student app URL scheme for deep links (e.g. "mobile" -> mobile://)
func AppScheme() string {
	return cfg.appScheme
}

// PlayStoreURL returns the Android Play Store URL for the student app
func PlayStoreURL() string {
	return cfg.playStoreURL
}

// AppStoreURL returns the iOS App Store URL for the student app
func AppStoreURL() string {
	return cfg.appStoreURL
}
...
// SMSOTPTemplate returns the OTP message template (one %s for OTP)
func SMSOTPTemplate() string {
	return cfg.smsOtpTemplate
}

// TwilioAccountSID returns the Twilio account SID
func TwilioAccountSID() string {
	return cfg.twilioAccountSid
}

// TwilioAuthToken returns the Twilio Auth Token (use with Account SID for SMS)
func TwilioAuthToken() string {
	return cfg.twilioAuthToken
}

// TwilioAPIKey returns the Twilio API Key SID (optional; use with API Secret for API Key auth)
func TwilioAPIKey() string {
	return cfg.twilioAPIKey
}

// TwilioAPISecret returns the Twilio API secret (API Key Secret)
func TwilioAPISecret() string {
	return cfg.twilioAPISecret
}

// TwilioPhoneNumber returns the Twilio "From" phone number for SMS (E.164)
func TwilioPhoneNumber() string {
	return cfg.twilioPhoneNumber
}
