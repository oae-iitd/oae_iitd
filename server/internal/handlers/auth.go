...
func Login(c *fiber.Ctx) error {
	...
	clientType := c.Get("X-Client")
	if clientType == "" {
		clientType = "web-admin"
	}

	// Authenticate user
	session, sessionID, err := auth.Login(ctx, req.Identifier, req.Password, clientType)
	if err != nil {
		if err == auth.ErrAccountInactive {
			return c.Status(403).JSON(fiber.Map{
				"error":   "account_not_active",
				"message": "Your account is pending admin approval or has been rejected.",
			})
		}
	...
	if !auth.IsRoleAllowedForClient(session.Role, clientType) {
		return c.Status(403).JSON(fiber.Map{
...
func Me(c *fiber.Ctx) error {
	...
	if rows.Next() {
		...
		if role, _ := userMap["role"].(string); !auth.IsRoleAllowedForClient(role, clientType) {
			log.Printf("[Me] Rejecting: X-Client=%s but user id=%v has role %q", clientType, userMap["_id"], role)
				return c.Status(403).JSON(fiber.Map{
					"error":   "forbidden",
					"message": "This account is not allowed on this platform.",
				})
...
func scanUserRowForMe(rows pgx.Rows) (fiber.Map, error) {
	...
	if ProfilePicture != nil {
		log.Printf("[Me] User %d has profilePicture: %s", ID, *ProfilePicture)
		userMap["profilePicture"] = *ProfilePicture
	} else {
		log.Printf("[Me] User %d has NO profilePicture in database", ID)
	}
...
// RevokeSession revokes a specific session (DELETE /api/sessions/:id, or JSON body sessionId).
func RevokeSession(c *fiber.Ctx) error {
	...
	// Mark session as logged out in database (instead of deleting)
	if err := database.MarkSessionLoggedOut(ctx, sessionID); err != nil {
...
func SendOTP(c *fiber.Ctx) error {
	...
	if err := cache.SetOTP(ctx, userEmail, otp); err != nil {
	...
	userID := user.ID
	if err := database.StoreOTP(ctx, userEmail, otp, &userID, "password_change"); err != nil {
	...
	// Send email with OTP
	if err := email.SendOTPEmail(ctx, userEmail, otp, &userID); err != nil {
		// Log error but don't fail the request - OTP is still stored and can be retrieved from logs
		log.Printf("[SendOTP] Warning: Failed to send email to %s: %v", userEmail, err)
		log.Printf("[SendOTP] OTP for %s: %s (Email failed, check server logs)", userEmail, otp)
