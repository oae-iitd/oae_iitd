...
func GetUsers(c *fiber.Ctx) error {
	...
	query := `
		SELECT id, COALESCE(username, '') AS username, COALESCE(email, '') AS email, role, phone, name, status, is_phone_verified,
...
// GetDrivers returns all drivers (accessible to all authenticated users)
func GetDrivers(c *fiber.Ctx) error {
	ctx, cancel := database.DefaultTimeout()
	defer cancel()

	query := `
		SELECT id, username, role, phone, name, status, is_phone_verified,
		       profile_picture, created_at, updated_at
		FROM users
		WHERE LOWER(role) = 'driver' AND status = 'active'
		ORDER BY name, id
	`

	rows, err := database.GetPool().Query(ctx, query)
	if err != nil {
		log.Printf("[GetDrivers] Query error: %v", err)
		return c.Status(500).JSON(fiber.Map{
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&ID, &Username, &Role, &Phone, &Name, &Status, &IsPhoneVerified,
			&ProfilePicture, &CreatedAt, &UpdatedAt,
		)
		if err != nil {
			log.Printf("[GetDrivers] Scan error: %v", err)
			continue
		}
		driverMap := fiber.Map{
			"_id":  strconv.Itoa(ID),
		...
		if Username != nil {
			driverMap["username"] = *Username
		}
		if Phone != nil {
			driverMap["phone"] = *Phone
		}
		if ProfilePicture != nil {
			driverMap["profilePicture"] = *ProfilePicture
		}
		drivers = append(drivers, driverMap)
	}

	if err := rows.Err(); err != nil {
		log.Printf("[GetDrivers] Rows error: %v", err)
		return c.Status(500).JSON(fiber.Map{
...
// GetDriversPublic returns a public driver list (no auth).
// Intended for pre-auth contexts where basic contact visibility is needed.
func GetDriversPublic(c *fiber.Ctx) error {
	ctx, cancel := database.DefaultTimeout()
	defer cancel()
	...
	rows, err := database.GetPool().Query(ctx, query)
	if err != nil {
		log.Printf("[GetDriversPublic] Query error: %v", err)
		return c.Status(500).JSON(fiber.Map{
			"error": "failed to fetch drivers",
		})
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ID, &Name, &Phone, &ProfilePicture); err != nil {
			log.Printf("[GetDriversPublic] Scan error: %v", err)
			continue
		}
		driverMap := fiber.Map{
			"_id": strconv.Itoa(ID),
	...
	if err := rows.Err(); err != nil {
		log.Printf("[GetDriversPublic] Rows error: %v", err)
		return c.Status(500).JSON(fiber.Map{
			"error": "failed to process drivers",
		})
	}
...
func scanUserRow(rows pgx.Rows) (fiber.Map, error) {
	...
	// Drivers authenticate using phone OTP; keep username/email absent in responses.
...
func isValidUserStatus(s string) bool {
	...
	case "active", "inactive", "expired", "closed":
		return true
	default:
		return false
	}
...
// normalizeIDProofTypeInput returns the stored id_proof_type value (camelCase for driverLicense).
func normalizeIDProofTypeInput(raw string) (string, bool) {
	low := strings.ToLower(strings.TrimSpace(raw))
	switch low {
	case "aadhaar", "pan", "voter", "passport":
		return low, true
	case "driverlicense":
...
func CreateUser(c *fiber.Ctx) error {
	...
	isDriver := roleLower == "driver"

	var username, email *string
	var hashedPassword *string
	if isDriver {
		// Driver: login via mobile/OTP only; username/email/password_hash stay NULL.
		if req.Phone == nil || strings.TrimSpace(*req.Phone) == "" {
			return c.Status(400).JSON(fiber.Map{
				"error": "phone is required for driver",
			})
		}
	} else {
	...
	err := database.GetPool().QueryRow(ctx, query,
		username, email, hashedPassword, req.Role, req.Phone, req.Name, status,
...
func UpdateUser(c *fiber.Ctx) error {
	...
	if req.IDProofType != nil {
		normalizedType, ok := normalizeIDProofTypeInput(*req.IDProofType)
		if !ok {
			return c.Status(400).JSON(fiber.Map{
				"error": "invalid id proof type. Must be one of: aadhaar, pan, voter, driverLicense, passport",
			})
